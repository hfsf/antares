...
class Connection:
    ...
    user-defined variables between two phenomenological models, translating
    them into native residual Equation nodes.
    """
    ...
    def __init__(
        ...
        # Format: list of tuples (eq_name, description, residual_expression_node)
        self._pending_links = []
    ...
    def _generate_specific_equation(self, source_var_name, sink_var_name):
        ...
        if not (isinstance(var_source, Variable) and isinstance(var_sink, Variable)):
            raise UnexpectedValueError(
                "Both specified attributes for a connection must be instances of the Variable class."
            )

        # Create the constraint expression in residual format (Source - Sink = 0)
        # Natively handled by the overloaded __sub__ in EquationNode
    ...
    def _generate_auto_connection_equations(self):
        ...
        and matches them with the sink_port by exact string name mapping.
        """
    ...
    def apply_to(self, target_model):
        ...
        (typically the Master Flowsheet). Delegates the instantiation to the
        target model to guarantee tensor-flattening safety within the CasADi graph.
