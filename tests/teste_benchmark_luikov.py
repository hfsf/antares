...
from antares.core.template_units import K, m, s
...
class BlocoMadeiraLuikov(Model):
    ...
    def DeclareVariables(self):
        ...
        self.M = self.createVariable(
            "M", "mol/m^3", "Umidade", exposure_type="differential"
        )
        self.M.distributeOn(self.malha_3d)

    def DeclareParameters(self):
        # Coeficientes fenomenológicos cruzados
        self.k11 = self.createParameter(
            "k11", "m^2/s", value=1.0e-5
        )  # Difusividade Térmica Principal
        self.k12 = self.createParameter(
            "k12", "K*m^5/(mol*s)", value=2.0e-6
        )  # Efeito Dufour
        self.k21 = self.createParameter(
            "k21", "mol/(m*K*s)", value=1.5e-6
        )  # Efeito Soret (Termodifusão)
        self.k22 = self.createParameter(
            "k22", "m^2/s", value=0.5e-5
        )  # Difusividade de Massa Principal
    ...
    def DeclareEquations(self):
        ...
        eq_calor = self.T.Diff() - (
            self.k11() * self.T.Div() + self.k12() * self.M.Div()
        )
        eq_massa = self.M.Diff() - (
            self.k21() * self.T.Div() + self.k22() * self.M.Div()
        )
        ...
        for face in faces:
            self.setBoundaryCondition(
                self.T, self.malha_3d, face, "dirichlet", self.T_amb()
            )
            self.setBoundaryCondition(
                self.M, self.malha_3d, face, "dirichlet", self.M_amb()
            )
...
if __name__ == "__main__":
    ...
    plotador.plot(
        ...
        coordinates=[
            (0.05, 0.05, 0.05)
        ],  # Exatamente no centro geométrico (Length = 0.1m)
